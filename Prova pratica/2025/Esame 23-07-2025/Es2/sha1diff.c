...
void process_diff(const char *src, const char *dest) {
    DIR *dir = opendir(src);
    if (!dir) return;
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (lstat(src_path, &st) == 0) {
            if (S_ISDIR(st.st_mode)) {
                process_diff(src_path, dest_path);
            } 
            else if (S_ISREG(st.st_mode)) {
                calculate_hash(src_path, current_hash);
                
                if (current_hash[0] != '\0') {
                    int fd = open(dest_path, O_RDONLY);
                    if (fd != -1) {
                        int bytes_read = read(fd, stored_hash, 40);
                        close(fd);
